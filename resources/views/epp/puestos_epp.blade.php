...
<style>
  ...
  .table-scroll::-webkit-scrollbar-thumb{background:#c7c7d1;border-radius:8px}
            ...
            <div class="relative max-h-[70vh] overflow-auto border rounded-2xl shadow-sm table-scroll">
                <table class="min-w-full text-sm">
                    <thead class="bg-indigo-600 text-white">
                        <tr>
                            {{-- Esquina: sticky arriba + izquierda, z alto --}}
                            <th class="sticky top-0 left-0 z-40 p-3 text-left bg-indigo-600">
                            Puesto de trabajo
                            </th>
                    ...
                    <tbody>
                        @foreach($puestos as $row)
                            <tr class="odd:bg-white even:bg-gray-50 hover:bg-indigo-50/40">
                                ...
                                <td class="left-0 z-20 bg-white p-3 font-medium text-gray-800 border-r">
                                    {{ $row->departamento }}
                                </td>
                                <input type="hidden" name="puestos_visibles[]" value="{{ $row->id_puesto_trabajo }}">
                                @foreach($epps as $col)
                                    @php
                                    $checked = isset($pivot[$row->id_puesto_trabajo]) && isset($pivot[$row->id_puesto_trabajo][$col->id_epp]);
                                    @endphp
                                    <td class="p-2 text-center align-middle border-l">
                                        <input type="checkbox"
                                        name="matrix[{{ $row->id_puesto_trabajo }}][{{ $col->id_epp }}]"
                                        value="1"
                                        class="row-{{ $row->id_puesto_trabajo }} col-{{ $col->id_epp }} rounded w-5 h-5 cursor-pointer focus:ring-indigo-500"
                                        @checked($checked)
                                        />
                                    </td>
                                @endforeach
    ...
    <script>
        ...
        const marcarTodo = document.getElementById('marcarTodo');
        const desmarcarTodo = document.getElementById('desmarcarTodo');
        ...
        // Opcional: alternar ancho de columnas
