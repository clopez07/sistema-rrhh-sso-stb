...
@section('content')
    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <div class="flex items-center">
            ...
            <span class="text-sm font-medium text-black">Registro de Asistencia</span>
        </div>
        </li>
    </ol>
    </nav>
    ...
    @endif
    @if(session('warning'))
        <div class="mb-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3">{{ session('warning') }}</div>
    @endif
...
<div class="flex items-center justify-between w-full mb-4">
        ...
        <div id="crud-modal" tabindex="-1" aria-hidden="true" 
            ...
            <div class="relative p-4 w-full max-w-3xl max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow-sm">
                    ...
                    <!-- Modal body -->
                    <form action="{{ route('asistencias.store') }}" method="POST" class="p-4 md:p-5 space-y-6">
                        @csrf
                        <!-- Empleados -->
                        <div>
                            <label class="block text-sm font-medium text-gray-900 mb-2">
                                Seleccione los nombres de los empleados
                            </label>
                            ...
                            <div class="flex gap-2">
                                ...
                                <div class="flex flex-col justify-center gap-1">
                                    <button type="button" 
                                        onclick="mover('id_empleado', 'empleados-seleccionados')" 
                                        class="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-sm">&gt;</button>
                                    <button type="button" 
                                        onclick="mover('empleados-seleccionados', 'id_empleado')" 
                                        class="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-sm">&lt;</button>
                                    <button type="button" 
                                        onclick="moverTodos('id_empleado', 'empleados-seleccionados')" 
                                        class="bg-green-500 hover:bg-green-600 text-white px-2 py-1 rounded text-sm">Todos &gt;&gt;</button>
                                    <button type="button" 
                                        onclick="moverTodos('empleados-seleccionados', 'id_empleado')" 
                                        class="bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded text-sm">&lt;&lt; Todos</button>
                                </div>
                                
                                <!-- Lista derecha -->
                                <select name="empleados[]" id="empleados-seleccionados" multiple required 
                        ...
                        <div>
                            ...
                            <input type="text" id="buscar-capacitacion" placeholder="Buscar capacitación"
                                class="w-full mb-2 p-2 text-sm border border-gray-300 rounded-lg" />

                            <div class="flex gap-2">
                                <!-- Lista izquierda -->
                                <select id="id_capacitacion" multiple 
                                    class="w-1/2 h-40 border border-gray-300 rounded-lg p-1 text-sm">
                                    @foreach($capinstructor as $capinstructo)
                                        <option value="{{ $capinstructo->id_capacitacion_instructor }}">
                                            {{ $capinstructo->capacitacion }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div class="flex flex-col justify-center gap-1">
                                    <button type="button" 
                                        onclick="mover('id_capacitacion', 'capacitaciones-seleccionadas')" 
                                        class="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-sm">&gt;</button>
                                    <button type="button" 
                                        onclick="mover('capacitaciones-seleccionadas', 'id_capacitacion')" 
                                        class="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-sm">&lt;</button>
                                </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="instructor" class="block text-sm font-medium text-gray-900">
                            ...
                            <div>
                                <label for="fecha" class="block text-sm font-medium text-gray-900">
                                    Fecha de entrega
                                </label>
            ...
            <script>
                ...
                filtrarSelect("buscar-capacitacion", "id_capacitacion");
                </script>
        </div>
 
        <!-- Botón subir lista -->
        <form id="importForm" action="{{ route('asistencia.import') }}" method="POST" enctype="multipart/form-data"
            style="display: none;">
            @csrf
    ...
    <form action="{{ route('asistencia') }}" method="GET" class="relative w-full max-w-sm bg-white flex items-center">
    ...
    <div class="relative overflow-x-auto overflow-y-auto max-h-[70vh] shadow-md sm:rounded-lg">
        ...
        <thead class="sticky top-0 z-20 bg-gray-50">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b border-gray-200 hover:bg-gray-50">
                    <td class="px-6 py-4">
                        {{ $asistenci->nombre_completo }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $asistenci->codigo_empleado }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $asistenci->puesto_trabajo }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $asistenci->departamento }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $asistenci->capacitacion }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $asistenci->instructor }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $asistenci->fecha_recibida }}
                    </td>
                    <td class="flex items-center px-6 py-4">
                        <button data-modal-target="popup-modal-{{ $asistenci->id_asistencia_capacitacion }}" data-modal-toggle="popup-modal-{{ $asistenci->id_asistencia_capacitacion }}" class="font-medium text-red-600 hover:underline ms-3">Eliminar</button>

                        <div id="popup-modal-{{ $asistenci->id_asistencia_capacitacion }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow-sm">
                                    <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="popup-modal-{{ $asistenci->id_asistencia_capacitacion }}">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <form action="{{ route('asistencia.destroyasistencia', $asistenci->id_asistencia_capacitacion) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                                Sí, estoy seguro
                                            </button>
                                            <button data-modal-hide="popup-modal-{{ $asistenci->id_asistencia_capacitacion }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100">
                                                No, cancelar
                                            </button>
                                        </form>
                                    </div>
                        ...
                        <a href="{{ route('capacitaciones.export', $asistenci->id_empleado) }}" 
                        class="font-medium text-blue-600 hover:underline ms-3">
                        Imprimir Registro de Capacitaciones
                        </a>
                    </td>
                </tr>
                @endforeach
