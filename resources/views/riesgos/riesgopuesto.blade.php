...
@section('content')
    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <div class="flex items-center">
            ...
            <span class="text-sm font-medium text-black">Información de Matriz de Riesgos</span>
        </div>
        </li>
    </ol>
    </nav>
...
<!-- Form oculto -->
<form id="importForm" action="{{ route('riesgos.import') }}" method="POST" enctype="multipart/form-data" style="display:none;">
    @csrf
    ...
    <div id="create-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm">
                ...
                <!-- Modal body -->
                <form action="{{ route('riesgopuesto.storeriesgopuesto') }}" method="POST" class="p-4 md:p-5">
                    @csrf  
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        ...
                        <div class="col-span-2">
                            <label for="id_riesgo" class="block mb-2 text-sm font-medium text-gray-900">Seleccione el Riesgo</label>
                            <select name="id_riesgo" id="id_riesgo" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" required>
                                <option value="" disabled selected>Seleccione el tipo de Riesgo</option>
                                @foreach($riesgo as $riesgos)
                                    <option value="{{ $riesgos->id_riesgo }}">{{ $riesgos->nombre_riesgo }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-span-2">
    ...
    <form action="{{ route('riesgopuesto') }}" method="GET" class="relative w-full max-w-sm bg-white flex items-center">
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg" style="margin-top: 20px;">
        <table id="tablaCapacitaciones" class="w-full text-sm text-left rtl:text-right text-gray-500">
            ...
            <tbody>
                @foreach ($riesgopuesto as $riesgopuestos)
                <tr class="bg-white border-b border-gray-200 hover:bg-gray-50">
                    <td class="px-6 py-4">
                        {{ $riesgopuestos->puesto_trabajo_matriz }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $riesgopuestos->nombre_riesgo }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $riesgopuestos->valor }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $riesgopuestos->observaciones }}
                    </td>
                    <td class="flex items-center px-6 py-4">
                        <button data-modal-target="edit-modal-{{ $riesgopuestos->id_riesgo_valor }}" data-modal-toggle="edit-modal-{{ $riesgopuestos->id_riesgo_valor }}" class="font-medium text-blue-600 hover:underline">Editar</button>

                        <!-- Main modal -->
                        <div id="edit-modal-{{ $riesgopuestos->id_riesgo_valor }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <!-- Modal content -->
                                <div class="relative bg-white rounded-lg shadow-sm">
                                    <!-- Modal header -->
                                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-200">
                                        ...
                                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-toggle="edit-modal-{{ $riesgopuestos->id_riesgo_valor }}">
                                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                            </svg>
                                            <span class="sr-only">Close modal</span>
                                    ...
                                    <!-- Modal body -->
                                    <form action="{{ route('riesgopuesto.updateriesgopuesto', $riesgopuestos->id_riesgo_valor) }}" method="POST" class="p-4 md:p-5">
                                        @csrf
                                        @method('PUT')
                                        <div class="grid gap-4 mb-4 grid-cols-2">
                                            <div class="col-span-2">
                                                ...
                                                <select name="id_puesto_trabajo_matriz" id="id_puesto_trabajo_matriz" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" required>
                                                    <option value="" disabled selected>Seleccione el puesto de Trabajo</option>
                                                    @foreach($puestos as $puesto)
                                                        <option value="{{ $puesto->id_puesto_trabajo_matriz }}" @if($riesgopuestos->id_puesto_trabajo_matriz == $puesto->id_puesto_trabajo_matriz) selected @endif>
                                            ...
                                            <div class="col-span-2">
                                                ...
                                                <select name="id_riesgo" id="id_riesgo" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" required>
                                                    <option value="" disabled selected>Seleccione el tipo de Riesgo</option>
                                                    @foreach($riesgo as $riesgos)
                                                        <option value="{{ $riesgos->id_riesgo }}" @if($riesgopuestos->id_riesgo == $riesgos->id_riesgo) selected @endif>
                                                            {{ $riesgos->nombre_riesgo }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-span-2">
                                                <label for="valor" class="block mb-2 text-sm font-medium text-gray-900">Valor del Riesgo</label>
                                                <input type="text" name="valor" id="valor" value="{{ $riesgopuestos->valor }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" required="">
                                            </div>
                                            <div class="col-span-2">
                                                <label for="observaciones" class="block mb-2 text-sm font-medium text-gray-900">Observaciones</label>
                                                <input type="text" name="observaciones" id="observaciones" value="{{ $riesgopuestos->observaciones }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                                            </div>
                                        </div>
                                        <button type="submit" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                                            <svg class="me-1 -ms-1 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd"></path></svg>
                        ...
                        <button data-modal-target="popup-modal-{{ $riesgopuestos->id_riesgo_valor }}" data-modal-toggle="popup-modal-{{ $riesgopuestos->id_riesgo_valor }}" class="font-medium text-red-600 hover:underline ms-3">Eliminar</button>

                        <div id="popup-modal-{{ $riesgopuestos->id_riesgo_valor }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow-sm">
                                    <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="popup-modal-{{ $riesgopuestos->id_riesgo_valor }}">
                                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                        </svg>
                                        <span class="sr-only">Close modal</span>
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <form action="{{ route('riesgopuesto.destroyriesgopuesto', $riesgopuestos->id_riesgo_valor) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                                Sí, estoy seguro
                                            </button>
                                            <button data-modal-hide="popup-modal-{{ $riesgopuestos->id_riesgo_valor }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100">
                                                No, cancelar
                                            </button>
                                        </form>
                                    </div>
