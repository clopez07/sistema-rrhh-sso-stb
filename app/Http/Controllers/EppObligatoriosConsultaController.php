...
public function index(Request $request)
{
    ...
    $puestos = collect($puestosMatriz)
        ...
        ->sortBy(fn($row) => strtolower($row->nombre ?? ''))
        ->values();
    ...
    if ($puestoTipo && $puestoId) {
        ...
        $empleados = $empleadosQuery->orderBy('nombre_completo')->get();
        ...
        if ($eppsObligatorios->count() && $empleados->count()) {
            ...
            // ====== Parser robusto (como ya lo tenías) ======
...
";
    ...
    STR_TO_DATE($isoToken, '%Y-%m-%d')
            ...
            // ⬇️ Partimos por permanencia
            $idsPermanentes = $eppsObligatorios
                ->filter(fn($e) => strcasecmp(trim((string)$e->permanente), 'SI') === 0)
                ->pluck('id_epp')->all();

            $idsNoPermanentes = $eppsObligatorios
                ->filter(fn($e) => strcasecmp(trim((string)$e->permanente), 'SI') !== 0)
                ->pluck('id_epp')->all();

            $asigPerm = collect();
            if (!empty($idsPermanentes)) {
                $asigPerm = DB::table('asignacion_epp as asig')
                    ->select('asig.id_epp', 'asig.id_empleado')
                    ->selectRaw("DATE_FORMAT(MAX($parsed), '%Y-%m-%d') as fecha")
                    ->whereIn('asig.id_epp', $idsPermanentes)
                    ->whereIn('asig.id_empleado', $empleadoIds)
                    ->whereRaw("$parsed IS NOT NULL")
                    ->groupBy('asig.id_epp', 'asig.id_empleado')
                    ->get();
            }

            $asigNoPerm = collect();
            if (!empty($idsNoPermanentes)) {
                $asigNoPerm = DB::table('asignacion_epp as asig')
                    ->select('asig.id_epp', 'asig.id_empleado')
                    ->selectRaw("DATE_FORMAT(MAX($parsed), '%Y-%m-%d') as fecha")
                    ->whereIn('asig.id_epp', $idsNoPermanentes)
                    ->whereIn('asig.id_empleado', $empleadoIds)
                    ->whereRaw("$parsed IS NOT NULL AND YEAR($parsed) = ?", [$anio])
                    ->groupBy('asig.id_epp', 'asig.id_empleado')
                    ->get();
            }

            $asignaciones = $asigPerm->concat($asigNoPerm);
        }
...
public function export(Request $request)
{
    ...
    $eppsObligatorios = $eppsQuery
        ->orderBy('e.equipo')
        ->get(['e.id_epp', 'e.equipo', 'e.codigo', 'e.marca', 'e.permanente']);
    ...
    if ($eppsObligatorios->count() && $empleados->count()) {
        ...
        $asig = $asigPerm->concat($asigNoPerm);
