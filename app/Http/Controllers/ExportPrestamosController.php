    ...
    public function export()
    {
        ...
        cuotas AS (
            SELECT
                ...
                /* Última fila en el rango (global asc/desc para rellenos) */
                ROW_NUMBER() OVER (
                ...
                /* Última fila global por fecha (para fallbacks) */
                ROW_NUMBER() OVER (
                    PARTITION BY cr.id_prestamo
                    ORDER BY cr.fecha_programada DESC, cr.id_historial_cuotas DESC
                ) AS rn_desc_global,

                /* Marca filas con avance real (capital/interés) y posición de la más reciente */
                CASE
                    WHEN COALESCE(cr.saldo_pagado,0) > 0
                    OR COALESCE(cr.interes_pagado,0) > 0
                    THEN 1 ELSE 0
                END AS has_progress,
                ...
                ROW_NUMBER() OVER (
                    ...
                    CASE
                        ...
                        THEN 0 ELSE 1
                    END,
                    cr.fecha_programada DESC,
                    cr.id_historial_cuotas DESC
                ) AS rn_ult_pagada
            FROM cuotas_raw cr
            LEFT JOIN qslots qs
                ON qs.fecha_q = cr.fecha_programada
        ),
        res AS (
            SELECT
                ...
                /* === TOTALES AUTOMÁTICOS SEGÚN LA ÚLTIMA CUOTA PAGADA === */
                COALESCE(
                MAX(CASE WHEN rn_ult_pagada = 1 THEN saldo_pagado END),
                0
                ) AS `CAPITAL PAGADO`,

                COALESCE(
                MAX(CASE WHEN rn_ult_pagada = 1 THEN saldo_restante END),
                MAX(CASE WHEN rn_desc_global  = 1 THEN saldo_restante END),
                MAX(MONTO)
                ) AS `CAPITAL PENDIENTE`,

                COALESCE(
                MAX(CASE WHEN rn_ult_pagada = 1 THEN interes_pagado END),
                0
                ) AS `INTERESES PAGADOS`,

                COALESCE(
                MAX(CASE WHEN rn_ult_pagada = 1 THEN interes_restante END),
                MAX(INTERES),
                0
                ) AS `INTERESES PENDIENTES`,
        ...
        SELECT
            ...
            /* Mantengo el orden final como lo tenías: OBS + QIII al final en el UNION,
            pero los valores se acceden por clave en PHP, así que no afecta */
            `CAPITAL PAGADO`, `CAPITAL PENDIENTE`, `INTERESES PAGADOS`, `INTERESES PENDIENTES`,
            `OBSERVACIONES`, `QUINCENA III`, `FECHA III`
        FROM res
        UNION ALL
        SELECT
            'TOTAL', NULL, NULL, NULL, NULL, NULL,
            SUM(MONTO), SUM(INTERES), SUM(`CAPITAL MENSUAL PAGADO`), SUM(`INTERES MENSUAL`), SUM(`CUOTA MENSUAL`),
